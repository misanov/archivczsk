...
class ArchivUpdater(object):
    def __init__(self, archivInstance):
        ...
        self.updateXml = "http://cdn.rawgit.com/misanov/archivczsk/{commit}/build/plugin/update/app.xml"
        self.updateZip = "http://cdn.rawgit.com/misanov/archivczsk/{commit}/build/plugin/update/version/archivczsk-{version}.zip"
        self.commit = "https://raw.githubusercontent.com/misanov/archivczsk/master/build/plugin/update/commit"
...
class Updater(object):
    ...
    def check_addons(self, new=True):
        ...
        for addon_id in self.repository._addons:
            if addon_id not in self.remote_addons_dict.keys():
                local_addon = self.repository.get_addon(addon_id)
